...
> 🧹 Post-processing (largest connected component) maintained high quality at **Dice = 0.8482**.
> 📈 Data augmentation improved raw test Dice from **0.50 → 0.8741** (+0.37).
...
Train.ipynb
  ...
  → 100 epochs on CPU
  → Best model saved: best_metric_model.pth (epoch 96, Dice = 0.8741)
        ↓
Testing.ipynb
  → Load best model
...
│   ├── Train.ipynb               # U-Net training loop
...
```text
Preparation_nii.ipynb → PreProcess_train.ipynb → Train.ipynb → Testing.ipynb → PostProcessing.ipynb
```
---
## 📦 Dependencies
```text
